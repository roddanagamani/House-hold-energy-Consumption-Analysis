...
HAVING COUNT(l.like_id) > (
    SELECT AVG(likes_per_post)
    FROM (
        SELECT COUNT(*) AS likes_per_post
        FROM Likes
        GROUP BY post_id
    ) AS subquery
);
...
SELECT DISTINCT p.post_id, p.content, u.username AS friend_name
FROM Friendships f
JOIN Users u ON (u.user_id = f.user_id1 OR u.user_id = f.user_id2)
JOIN Likes l ON u.user_id = l.user_id
...
SELECT u.username, 
       COALESCE(l.total_likes,0) + COALESCE(c.total_comments,0) AS influence_score
FROM Users u
LEFT JOIN (
